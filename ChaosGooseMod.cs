// ChaosGooseMod.cs — 5 Stage Virus Goose
//
// Compile command:
...
{
    private int _stage = 0;          // 0 = idle, 1-5 = chaos stages
    ...
    private void OnGooseClicked(GooseEntity goose)
    {
        ...
        // Each click advances the stage (max 5)
        int newStage = Math.Min(5, _clickCount);
        ...
        // Goose gets progressively angrier
        GooseEntity.SpeedTiers tier = _stage >= 4
            ? GooseEntity.SpeedTiers.Charge
            : _stage >= 2
                ? GooseEntity.SpeedTiers.Run
